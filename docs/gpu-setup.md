# 🎮 GPU加速化セットアップガイド (Windows + WSL2)

## 概要

このガイドでは、**Windows + WSL2 環境** での vyb-code GPU加速設定手順を説明します。GPU加速により、qwen2.5-coder:14b モデルで **78%のパフォーマンス向上** (13.4秒 → 3.2秒) を実現できます。

> **重要**: このガイドは Windows + WSL2 ユーザー専用です。macOS や Linux ネイティブ環境では設定手順が異なります。
